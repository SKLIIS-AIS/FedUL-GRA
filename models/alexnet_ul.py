...
class AlexNet_UL_Independent(nn.Module):
    def __init__(self, num_classes, in_channels):
        super().__init__()
        self.num_classes = num_classes
        ...
        self.features_ul = self._make_features(in_channels)
        self.classifier = nn.Linear(4 * 4 * 256, num_classes)
        self.classifier_ul = nn.Linear(4 * 4 * 256, num_classes)
    ...
    def forward(self, x, mode=None):
        ...
        raise ValueError('Unknown AlexNet_UL_Independent mode: {}'.format(mode))


def alexnet_ul_independent(**kwargs):
    model = AlexNet_UL_Independent(**kwargs)
    return model
