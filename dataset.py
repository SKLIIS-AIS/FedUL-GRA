...
class UL_CelebA(VisionDataset):
    """CelebA wrapper for binary and 10-class attribute-combination tasks."""

    DEFAULT_ATTRS10 = ('Smiling', 'Male', 'Young', 'Eyeglasses')
    ...
    def __init__(
        ...
        super(UL_CelebA, self).__init__(root, transform=transform,
                                        target_transform=target_transform)

        self.train = train
        self.ul_mode = ul_mode or 'none'
        self.attr_name = attr_name
        self.ul_class_id = int(ul_class_id)
        ...
        self.attr_names = list(getattr(self.dataset, 'attr_names', []))

        self.active_indices = None
        if self.target_mode == 'binary':
            if attr_name not in self.attr_names:
                raise ValueError(
                    'Unknown CelebA attribute: {}. Available attributes include: {}'.format(
                        attr_name,
                        ', '.join(self.attr_names[:10]),
                    )
                )
            self.attr_idx = self.attr_names.index(attr_name)
            self.targets = (self.dataset.attr[:, self.attr_idx] > 0).numpy().astype(np.int64)
            self.classes = [0, 1]
            self.active_indices = list(range(len(self.targets)))
            self.attrs10_attrs = []
            self.combo_to_class = {}
        elif self.target_mode == 'attrs10':
            self.attr_idx = None
            self.targets, self.active_indices, self.combo_to_class, self.attrs10_attrs = self._build_attrs10_targets(root, attr_name)
            self.classes = list(range(10))
        else:
            raise ValueError('Unknown CelebA target mode: {}. Use binary or attrs10.'.format(self.target_mode))

        self.num_classes = len(self.classes)
        if not 0 <= self.ul_class_id < self.num_classes:
            raise ValueError(
                'CelebA {} target uses class ids [0, {}]; got --ul_class_id {}.'.format(
                    self.target_mode,
                    self.num_classes - 1,
                    self.ul_class_id,
                )
        ...
        self.ul_class_idxs = []
        self.ul_sample_idxs = list(private_idxs)
        self.final_train_list = []

        common_idxs = []
        for idx, target in enumerate(self.targets):
            if 'class' in self.ul_mode:
                if int(target) == int(ul_class_id):
                    self.ul_class_idxs.append(idx)
                else:
                    common_idxs.append(idx)

        if 'class' in self.ul_mode:
            if not self.ul_class_idxs:
                raise ValueError(
                    'CelebA split {} has no samples for attribute {}={}.'.format(
                        self.split,
            ...
            proportion = 1.0
            self.ul_sample_idxs = random.sample(
                self.ul_class_idxs,
                int(proportion * len(self.ul_class_idxs)),
            )
            print('CelebA target:', self._target_description())
            print('self.ul_sample_idxs:', len(self.ul_sample_idxs))
            self.final_train_list = list(set(self.ul_sample_idxs).union(set(common_idxs)))
        else:
            self.final_train_list = [i for i in range(len(self.targets))]

        self.ul_sample_idx_set = set(int(idx) for idx in self.ul_sample_idxs)
        self.sample_fake_targets = {}
        if 'samples' in self.ul_mode:
            candidates = [class_id for class_id in self.classes if class_id != self.ul_class_id]
            if not candidates:
                raise ValueError('CelebA sample unlearning needs at least two classes.')
            for idx in self.ul_sample_idx_set:
                self.sample_fake_targets[idx] = random.choice(candidates)

    def __len__(self):
        return len(self.targets)

    def _parse_attrs10(self, attr_name):
        attrs = [item.strip() for item in attr_name.split(',') if item.strip()]
        if len(attrs) < 2:
            attrs = list(self.DEFAULT_ATTRS10)
        missing = [attr for attr in attrs if attr not in self.attr_names]
        if missing:
            raise ValueError(
                'Unknown CelebA attrs10 attribute(s): {}. Available attributes include: {}'.format(
                    ', '.join(missing),
                    ', '.join(self.attr_names[:10]),
                )
            )
    ...
    def _combo_rows(self, dataset, attrs):
        attr_indices = [self.attr_names.index(attr) for attr in attrs]
        return (dataset.attr[:, attr_indices] > 0).numpy().astype(np.int64)

    def _build_attrs10_targets(self, root, attr_name):
        attrs = self._parse_attrs10(attr_name)
        ...
        train_rows = self._combo_rows(train_dataset, attrs)
        counts = {}
        for row in train_rows:
            combo = tuple(int(value) for value in row)
            counts[combo] = counts.get(combo, 0) + 1
        top_combos = sorted(counts.items(), key=lambda item: (-item[1], item[0]))[:10]
        if len(top_combos) < 10:
            raise ValueError(
                'CelebA attrs10 needs at least 10 observed attribute combinations; found {}.'.format(
                    len(top_combos)
                )
            )
        combo_to_class = {combo: class_id for class_id, (combo, _count) in enumerate(top_combos)}

        split_rows = self._combo_rows(self.dataset, attrs)
        active_indices = []
        targets = []
        ...
        print('CelebA attrs10 attributes:', ','.join(attrs))
        print('CelebA attrs10 train class counts:', [count for _combo, count in top_combos])
        print('CelebA attrs10 {} kept samples: {}'.format(self.split, len(active_indices)))
        return np.array(targets, dtype=np.int64), active_indices, combo_to_class, attrs

    def _target_description(self):
        if self.target_mode == 'attrs10':
            return 'attrs10({})'.format(','.join(self.attrs10_attrs))
        return self.attr_name

    def _add_trigger(self, img):
        img = img.copy()
        draw = ImageDraw.Draw(img)
        square_size = max(8, min(img.size) // 5)
        draw.rectangle((0, 0, square_size, square_size), fill=(255, 255, 255))
        return img

    def __getitem__(self, index):
        raw_index = self.active_indices[index]
        img, _attr = self.dataset[raw_index]
        target = int(self.targets[index])

        if 'samples' in self.ul_mode and index in self.ul_sample_idx_set:
            img = self._add_trigger(img)
            backdoor_target = self.ul_class_id
            fake_target = self.sample_fake_targets[index]
            target = backdoor_target + self.num_classes * (fake_target + 1)
